...
def parameters():
    ...
    if request.method == 'POST':
        ...
        waiter = 0
        wait = 100 / Steps
        specdata = np.loadtxt(fff, dtype=(np.float_), usecols=(0, 1), skiprows=1)
        ...
        R, T, wlengths, hv, N_0, V_bi, alpha_n, alpha_p = solar.valvalues(Ta, E_gn, E_gp, D_n, D_p, eps_p, eps_n, N_a, N_d, N_cp, N_vp, N_cn, N_vn, S_n, S_p, W_n, W_p, W_pmin, W_pmax, X_n, X_p, S_i, Steps, Vib_steps, L_n, L_p, wlengthsneq, N_0neq, Rneq, Tneq, nn)
        ...
        bb = Jone[(len(Vone) - 1)]
        cc = voltage[len(Vone)]
        dd = solar.Jcell(voltage[len(Vone)])
        V_oc = solar.Voc(aa, bb, cc, dd)
        Vone.append(V_oc)
        Jone.append(0.0)
...
def graphs():
    if wow == True:
        ...
        plt.plot(Vone*1e3, Jone, linewidth=0.8)
        plt.plot(Vone*1e3, Jone, 'o', linewidth=0.8)
...
def graphjsc():
    if wow == True:
        plt.figure(figsize=(8, 6), dpi=300, facecolor='w', edgecolor='k')
        plt.plot((data_1[0] * 1.0e7), (data_1[4]), linewidth=0.8)
        plt.plot(data_1[0] * 1.0e7, data_1[4], 'o')
...
def graphvoc():
    if wow == True:
        plt.figure(figsize=(8, 6), dpi=300, facecolor='w', edgecolor='k')
        plt.plot((data_1[0] * 1.0e7), (data_1[2]*1e3), linewidth=0.8)
        plt.plot(data_1[0] * 1.0e7, data_1[2]*1e3, 'o')
        plt.xlabel('Thickness [nm]', {'fontsize': 8})
...
def graphff():
    if wow == True:
        plt.figure(figsize=(8, 6), dpi=300, facecolor='w', edgecolor='k')
        plt.plot((data_1[0] * 1.0e7), (data_1[3]), linewidth=0.8)
        plt.plot(data_1[0] * 1.0e7, data_1[3], 'o')
        plt.xlabel('Thickness [nm]', {'fontsize': 8})
...
def grapheffi():
    if wow == True:
        plt.figure(figsize=(8, 6), dpi=300, facecolor='w', edgecolor='k')
        plt.plot((data_1[0] * 1.0e7), (data_1[1]), linewidth=0.8)
        plt.plot(data_1[0] * 1.0e7, data_1[1], 'o')
        plt.xlabel('Thickness [nm]', {'fontsize': 8})

...
E_gn = np.float_(2.42)
E_gp = np.float_(1.02)#me recuerda entonces
...
def dJ_p(V,W_p):
    x_p = functionx_p(V,W_p)
    x_n = functionx_n(V,W_p)
...
def dJ_ph(V,W_p):
    ans =dJ_p(V,W_p) + dJ_scr(V,W_p) + dJ_n(V,W_p) + (dJ_win(V,W_p) + dJ_RCEn(V,W_p) + (dJ_RCEp(V,W_p))  + dJ_abs(V,W_p)) 
...
def Jdark(V,W_p):
    ans = ( J_0p(V,W_p) + J_0n(V,W_p) ) * ( np.exp( V / ( k_B * Ta ) ) - 1.0 ) + ( J_00(V,W_p) + J_000(V,W_p) ) * ( np.exp( V / ( 2 * k_B * Ta ) ) - 1.0 )  
    return ans

def Jcell(V,W_p):
    ans = intdJ_ph(V,W_p) - Jdark(V,W_p) 
    return ans

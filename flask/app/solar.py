...
W_p = np.float_(2.5e-4)
W_pmin = np.float_(0.5e-4)
W_pmax = np.float_(3.0e-4)
X_n = np.float_(4.3)
X_p = np.float_(4.235) #del material tipo p
Ta = np.float_(300)
S_i = np.float_(0)
Steps = int(30)
waiter = 0
...
data = np.genfromtxt("N0.txt")
wlengths = data[:,][:, 0]
N_0 = data[:,][:, 1]
hv = np.array(4.1356e-15 * c / wlengths, dtype=np.float_)
...
def dJ_ph(V):
    ans =dJ_p(V) + dJ_scr(V) + dJ_n(V) + 0*(dJ_win(V) + dJ_RCEn(V) + (dJ_RCEp(V))  + dJ_abs(V)) 
...
for WW in thickness:
    ...
    J_sc = max(jcell)
    FF = 100 * Efi / ( V_oc * J_sc )
    print('Efficiency: ',(Efi),'Voc: ',(V_oc),'Jsc: ',(J_sc),'FF: ',(FF) , 'W: ', WW)
    #print("es esteeee", Jdark(0.001))
    Efi_list.append(Efi)
